...
namespace GRPC_Service.Repositories
{
    public class PersonService : IPersonService
    {
        ...
        public async Task<Person> CreatePerson(Person person)
        {
            try
            {
                var ExistedPerson=GetPersonByNationalId(person);
                if (ExistedPerson != null)
                {
                    throw new Exception("duplicated nationalId");
                }
        ...
        public async Task<Person?> GetPersonByNationalId(Person person)
        {
            return await _dataContext.Persons.Where(x => x.NationalCode == person.NationalCode).FirstOrDefaultAsync();
