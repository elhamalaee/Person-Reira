...
namespace WebAPI.Controllers
{
    ...
    public class PersonController : Controller
    {
        ...
        public async Task<Persons?> GetPersonList()
        {
            try
            {
                var data = await _personServiceClient.GetAllPersonsList(new Empty { });
                return data;
            }
            catch
            {
        ...
        public async Task<PersonModel?> GetPersonByNationalId(Person person)
        {
            try
            {
                var request = new GetPersonRequest { nationalId  = person.NationalCode };
                var data = await _personServiceClient.GetPersonAsync(request);
                return data;
            }
            catch
